...
G = xr.concat(Gs, dim="snps").T[:, np.load("maf.npy") >= 0.05]
G['samples'] = samples
...
if what == "scan":
    ...
    for ii, name in enumerate(data0['measures'].columns.values):
        if ii % njobs != num:
            continue
        print("Processing: {}".format(name))
        dst_file = join(dst_folder, "null_measures_normal_" + name + "_mean_standardize.json")
        if os.path.exists(dst_file) or os.path.exists(dst_file + ".failed"):
            continue
        r = analysis_null(G, data0['measures'], data0['kinship'], name, 'normal', limix.qc.mean_standardize)
        if r is None:
            open(dst_file + ".failed", "w").write("")
        else:
            json.dump(r, open(dst_file, "w"))