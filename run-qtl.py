...
dst_folder = "3"
if "--dst-folder=3" in sys.argv:
    dst_folder = "3"
elif "--dst-folder=4" in sys.argv:
    dst_folder = "4"
elif "--dst-folder=5" in sys.argv:
    dst_folder = "5"
elif "--dst-folder=6" in sys.argv:
    dst_folder = "6"
...
def analysis_QTL(G, pheno, kinship, pheno_name, lik, pheno_norm):
    ...
    return dict(
        pv=list(asarray(model.variant_pvalues)),
        lik=lik,
        call=call,
        phenotype=phenotype_hex,
        kinship=kinship_hex,
        null_covariate_effsizes=list(asarray(model.null_covariate_effsizes)),
        variant_effsizes=list(asarray(model.variant_effsizes)),
        variant_effsizes_se=list(asarray(model.variant_effsizes_se)),
    )
...
for name in data0["measures"].columns.values:
    if isnumber(data0["measures"].loc[:, name]) and isdiscrete(
        data0["measures"].loc[:, name]
    ):
        if "--filter-zeros-out" in sys.argv:
            # Amelie said that some zeros might represent missing data or
            # aberrant errors.
            v = data0["measures"].loc[:, name]
            if v.min() == 0:
                data0["measures"].loc[v == 0, name] = np.nan
...
for i in tqdm(range(1, 22)):
    G = xr.open_dataarray(
        "arrayexpress/HS_shuf.hdf5", "/imputed_genotypes/chr{}".format(i)
    )
    f = h5py.File("arrayexpress/HS_shuf.hdf5", "r")
...
if what == "scan":
    for ii, name in enumerate(data0["measures"].columns.values):
        ...
        if "--lik=poisson" in sys.argv:
            dst_file = join(dst_folder, "scan_measures_poisson_" + name + ".json")
            fun = lambda: analysis_QTL(G, meas, kin, name, "poisson", lambda x: x)
        
        elif "--lik=lmm-qg" in sys.argv:
            fn = "scan_measures_normal_" + name + "_quantile_gaussianize.json"
            dst_file = join(dst_folder, fn)
            qc = limix.qc.quantile_gaussianize
            fun = lambda: analysis_QTL(G, meas, kin, name, 'normal', qc)
        
        else:
            raise ValueError("Set --lik option, properly.")
    ...
    for ii, name in enumerate(data0["measures"].columns.values):
        if ii % njobs != num:
            continue
        print("Processing: {}".format(name))
        dst_file = join(dst_folder, "null_measures_poisson_" + name + ".json")
        if os.path.exists(dst_file) or os.path.exists(dst_file + ".failed"):
            continue
        r = analysis_null(
            G, data0["measures"], data0["kinship"], name, "poisson", lambda x: x
        )
        if r is None:
            open(dst_file + ".failed", "w").write("")
        else:
